...
func (l *QueryTransaction) Run() {
	...
	if err := helper.COPO_DB.Table("tx_orders").
		...
		logx.WithContext(context).Errorf("Err : %s", err.Error())
	}
	logx.WithContext(context).Infof("启动处理交易中，未回调的提单，笔数：%d 笔", len(orders))
	...
	if len(orders) > 0 {
		for _, order := range orders {
			...
			if queryErr := helper.COPO_DB.Table("ch_channels").Where("code = ?", order.ChannelCode).Find(&channel); queryErr != nil {
				logx.WithContext(context).Errorf("queryErr: %#v", queryErr.Error)
			}

			url := fmt.Sprintf("%s:%s/api/proxy-pay-query", viper.Get("CHANNEL_HOST"), channel.ChannelPort)
			logx.WithContext(context).Infof("發送代付查询請求To渠道: %s。 url: %s", order.OrderNo, url)

			//異步調用-呼叫異步調用服務
			go func() {
				if proxyQueryRespVO, chnErr := orderService.CallChannel_ProxyQuery(&context, url, &order); chnErr != nil || proxyQueryRespVO.Code != "0" {
					logx.WithContext(context).Errorf("排程代付人工處理呼叫失敗: %s，Error: %s ，進行人工處理，提單狀態改失敗", order.OrderNo, chnErr.Error())
					logx.WithContext(context).Errorf("交易中代付提单：%s，状态查询异常 :%s ", order.OrderNo, chnErr.Error())
					order.Status = "30"
					...
					logx.WithContext(context).Infof("查询代付提单URL：%s，提单编号：%s，提单回传结果: checkResult=%#v", url, order.OrderNo, proxyQueryRespVO)
		...
		wg.Wait()
		logx.WithContext(context).Info("WaitGroup Finished")
	}
}
