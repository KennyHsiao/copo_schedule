...
func InterMerchantReport(db *gorm.DB, req *types.MerchantReportQueryRequest, ctx context.Context) (resp *types.MerchantReportQueryResponse, err error) {
	...
	selectX := "DATE_ADD(DATE_FORMAT(tx.created_at, '%Y-%m-%d %H:00:00') ,INTERVAL 8 HOUR) AS settlement_time," +
		...
		"CASE WHEN tx.`type` = 'XF' THEN IF(mmc.withdraw_handling_fee = 0 OR mmc.withdraw_handling_fee IS NULL,bsr.withdraw_handling_fee, mmc.withdraw_handling_fee) ELSE mcr.handling_fee END AS merchant_handling_fee," +
	...
	tx := db.
		...
		Joins("LEFT JOIN mc_merchant_currencies mmc ON tx.merchant_code = mmc.merchant_code AND tx.currency_code = mmc.currency_code")
